...
public class ActivityAIService {
	...
	private Recommendations processAIResponse(Activity activity, String aiResponse) {
		try {
			...
			// extracting all the json and setting up the values
			...
			return Recommendations.builder().activityId(activity.getId()).userId(activity.getUserId())
					.type(activity.getType().toString()).recommendation(fullAnalysis.toString())
					.improvements(improvements).suggestions(suggestions).safety(safety).build();
		} catch (Exception e) {
			e.printStackTrace();
	...
	private Recommendations createDefaultRecommendation(Activity activity) {
		return Recommendations.builder().activityId(activity.getId()).userId(activity.getUserId())
				.type(activity.getType().toString()).recommendation("Unable to generate detailed analysis")
				.improvements(Collections.singletonList("Continue with your current routine"))
				.suggestions(Collections.singletonList("Consider sonsulting a fitness consultant"))
				.safety(Arrays.asList("Stay Hydrated", "Always listen to your body", "Listen to  your body")).build();
	}
	...
	private List<String> extractImprovements(JsonNode improvementsNode) {
		...
		return improvements.isEmpty() ? Collections.singletonList("No specific impeovements probvided") : improvements;
	}
